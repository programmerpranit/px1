...
Run locally without installing: `go run . [file-or-dir]` — add `-no-open` to skip launching a browser, `-port N` to pick a port, `-f` to run in the foreground and block the shell (background is the default, see below), `-dev web` to serve `web/` from disk instead of the embedded copy (required when iterating on `web/src/*.js` without rebuilding the bundle each time).

**Background by default**: `daemon.go`/`daemon_unix.go`/`daemon_windows.go` re-exec the binary as a session-detached child (stdout/stderr to a temp log file), wait for it to report the URL it bound via a temp status file, print that, and exit — the invoking shell gets control back immediately instead of blocking for the life of the server. Pass `-f` to skip this and block the shell like a normal server process. Each instance still walks forward from its requested port if busy (`listen` in `main.go`), so launching against several workspaces just stacks onto the next free port. `-d` is accepted as a deprecated no-op for old muscle memory/scripts.

**Instance registry (`px1 ps` / `px1 kill <pid>` / `px1 kill-all`)**: `instances.go` maintains one JSON file per running instance (PID, URL, workspace root, start time) under `os.TempDir()/px1-instances/` — ephemeral runtime bookkeeping, not workspace state. `registerInstance`/`unregisterInstance` are called in `main.go` around the listener lifetime for both foreground and background-child processes; `listInstances` prunes any entry whose PID is no longer alive (self-healing against `kill -9` or a crash) via the platform `processAlive` in `daemon_unix.go`/`daemon_windows.go`. `px1 kill <pid>` sends SIGTERM on unix (routes through the normal graceful-shutdown signal handler in `main.go`) or `TerminateProcess` on Windows (no SIGTERM support there); `px1 kill-all` does this for every registered instance.
